...
class PpListener(ParseTreeListener):
    ...
    def exitInput0(self, ctx:PpParser.Input0Context):
        if self.quadruples[-1].has_operand():
            operand = self.quadruples[-1].pop_operand()
            operand_type = self.quadruples[-1].pop_type()
            if operand_type.struct_type != StructuredTypes.NONE:
                print(f"Semantic error: Cannot read into structured type"
                f" at {ctx.start.line}:{ctx.start.column}")
                exit()
            else:
                self.quadruples[-1].add_quadruple("read", None, None, operand)
